...
export class MysqlCache<Scheme> extends MysqlNative<Scheme> {
  ...
  protected async deleteCache (ids: string[], dataList: CoaMysql.SafePartial<Scheme>[], trx?: CoaMysql.Transaction) {
    const deleteIds = this.getDeleteIds(ids, dataList)
    trx ? trx.recordDeleteCache(deleteIds) : await this.redisCache.mDelete(deleteIds)
  }
